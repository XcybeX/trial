...
yum install -y easy-rsa  # since openvpn does not ship with that anymore
...
if [ $? -ne 1 ]; then
	...
	echo There was a problem with installation of easy-rsa
	roll_back
fi 
...
#tidy up
perl -p -i -e "s/;log-append /log-append /"  /etc/openvpn/server.conf
